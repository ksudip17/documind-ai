    ...
    <div className="min-h-screen bg-gray-950">
      ...
      <nav className="border-b border-gray-800 px-6 py-4 flex justify-between items-center">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 bg-violet-600 rounded-lg flex items-center justify-center text-sm font-bold">D</div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">{documents.length} document{documents.length !== 1 ? 's' : ''} uploaded</p>
          </div>
          <div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-500 text-sm">Upload a PDF, DOCX, or TXT file to get started</p>
          </div>
        ) : (
          <div className="space-y-3">
            {documents.map((doc) => (
              <div
                key={doc.id}
                onClick={() => doc.status === 'COMPLETED' && router.push(`/dashboard/${doc.id}`)}
                className={`bg-gray-900 border border-gray-800 rounded-xl p-5 flex justify-between items-center
                  ${doc.status === 'COMPLETED' ? 'cursor-pointer hover:border-violet-500/50 transition' : ''}`}
              >
                <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-4">
                  <span className="text-xs text-gray-500">{doc._count.queries} queries</span>
                  <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${statusColor(doc.status)}`}>
                    {doc.status}
                  </span>
                </div>
              </div>
